 ...
 */
async function proxyVideo(req: Request, res: Response): Promise<any> {
  const targetUrl = req.query.url as string;
  const referer = (req.query.referer as string) || '';
  const isDash = req.query.type === 'dash';
  const wrapLevel = req.query.wrap as string;
  const embedUrl = (req.query.embed_url as string) || '';  // Para re-extracción VOE en caso de 403

  if (!targetUrl) return res.status(400).send('Error: Falta URL');
  ...
  try {
    ...
    if (isM3u8) {
      ...
      const isValidM3u8 = typeof body === 'string' && (
      ...
      if (!isValidM3u8 && isVoe && embedUrl) {
          ...
          try {
              ...
              const freshResponse: AxiosResponse = await axios({
                  method: 'GET',
              ...
              const isNowValid = typeof body === 'string' && (
                  body.trimStart().startsWith('#EXTM3U') ||
                  body.includes('#EXT-X-') ||
                  body.includes('#EXTINF')
              );
              ...
              console.error('[Proxy] Error en re-extracción VOE:', (reExtractErr as Error).message);
              res.set('Content-Type', 'application/json');
      ...
      if (isDash) {
        ...
        body = body.split('\n').map(line => {
          ...
          if (trimmed.startsWith('#')) {
            if (trimmed.includes('URI="')) {
              return trimmed.replace(/URI="(.*?)"/gi, (match, uri) => {
                ...
                if (isStreamwish) return `URI="${absolute}"`;
                return `URI="${proxyBase}${encodeURIComponent(absolute)}"`;
              });
            }
            return line;
          ...
          if (isStreamwish) return absolute;
          return `${proxyBase}${encodeURIComponent(absolute)}`;
        }).join('\n');
    ...
    console.error(`[Proxy] Error para ${targetUrl}:`, (error as Error).message);
