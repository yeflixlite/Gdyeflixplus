...
function solvePowCustom(nonce, difficulty) {
    ...
    const prefix = nonce + ':';
    while (true) {
        if (wr(gr(yr(prefix + s))) >= difficulty)
            return String(s);
        s++;
    }
}
...
function normalizeUrl(url) {
    ...
    if (!match)
        throw new Error('ID de Filemoon no encontrado en la URL.');
    return {
...
function filterKeyParts(version, key_parts) {
    if (!version || !Array.isArray(key_parts))
        return key_parts || [];
    ...
    if (!indices)
        return key_parts;
    const [i, s] = indices;
    if (i < 1 || s < 1 || i > key_parts.length || s > key_parts.length)
        return [];
...
function decryptPlayback(data) {
    try {
        ...
        const decipher = crypto_1.default.createDecipheriv('aes-256-gcm', key, ivBuf);
...
async function extractViaApi(id, origin) {
    ...
    try {
        ...
        if (!detailsRes.data || !detailsRes.data.embed_frame_url)
            return null;
        ...
        const viewer_id = crypto_1.default.randomBytes(16).toString('hex');
        const device_id = crypto_1.default.randomBytes(16).toString('hex');
        ...
        const { publicKey, privateKey } = crypto_1.default.generateKeyPairSync('ec', { namedCurve: 'prime256v1' });
        const jwk = publicKey.export({ format: 'jwk' });
        const rawSignature = crypto_1.default.sign('SHA256', Buffer.from(challenge.nonce), {
            key: privateKey,
        ...
        if (!attRes.data || !attRes.data.token)
            return null;
