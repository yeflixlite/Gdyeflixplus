 ...
 */
function detectProvider(url: string): ProviderName {
  if (!url) return 'unknown';
  for (const [provider, patterns] of Object.entries(PROVIDERS)) {
    if (Array.isArray(patterns)) {
      for (const pattern of patterns) {
        if (pattern.test(url)) return provider as ProviderName;
      }
    }
  }
  return 'unknown';
 ...
 */
function extractVideoId(url: string, provider: ProviderName): string | null {
  try {
    const u = new URL(url);
    switch (provider) {
      case 'doodstream': {
