 ...
 */
function decodeVoeConfig(encoded: string): Record<string, string> | null {
    try {
        // 1. ROT13
        ...
        return JSON.parse(finalJson) as Record<string, string>;
    } catch (err) {
        console.error('[VOE] Error decodificando config:', (err as Error).message);
        return null;
    }
}
...
async function extract(url: string): Promise<ExtractResult> {
    ...
    const uniqueHosts: string[] = [...new Set(hostsToTry)];
            ...
            try {
                ...
                if (b64Match) {
                    ...
                    if (decoded.includes('.m3u8') || decoded.includes('.mp4')) {
                        const json = JSON.parse(decoded) as Record<string, string>;
