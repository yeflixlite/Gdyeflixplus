/**
 * ============================================================
 *  services/envivos/ksdExtractor.ts
 ...
 * @param channelId
 */
async function extractChannel(channelId: string): Promise<ExtractResult> {
    ...
    try {
        ...
        const res1  = await fetchWithRetry(embedUrl, { referer: 'https://google.com/' });
        ...
        const res2  = await fetchWithRetry(streamUrl, { referer: embedUrl });
        ...
        if (!m3u8Match) {
            ...
            if (jwMatch) {
                return { videoUrl: jwMatch[1], type: 'm3u8', referer: streamUrl };
            }
            throw new Error('No se encontró el enlace .m3u8 final');
        }

        return { videoUrl: m3u8Match[1], type: 'm3u8', referer: streamUrl };

    } catch (err) {
        throw new Error(`Fallo en ksdExtractor (${channelId}): ${(err as Error).message}`);
    }
}
