...
async function extract(url: string): Promise<ExtractResult> {
  ...
  if (DIRECT_EXTENSIONS.test(url)) {
    const type: VideoType = url.includes('.m3u8') ? 'm3u8' : 'mp4';
  ...
  const videoSrc: string | undefined = $('video source, video').attr('src');
  if (videoSrc && videoSrc.startsWith('http')) {
    const type: VideoType = videoSrc.includes('.m3u8') ? 'm3u8' : 'mp4';
    console.log(`[Generic] ✔ <video src> encontrado`);
    return { videoUrl: videoSrc, type, referer: origin };
  }

  // ── Estrategia 2: og:video meta ──────────────────────────────
  const ogVideo: string | undefined = $('meta[property="og:video:url"], meta[property="og:video"]').attr('content');
  if (ogVideo && ogVideo.startsWith('http')) {
    const type: VideoType = ogVideo.includes('.m3u8') ? 'm3u8' : 'mp4';
  ...
  if (fileMatch) {
    ...
    const type: VideoType = videoUrl.includes('.m3u8') ? 'm3u8' : 'mp4';
