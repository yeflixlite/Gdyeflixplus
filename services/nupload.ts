...
async function extract(url: string): Promise<ExtractResult> {
  try {
    const headers: Record<string, string> = getBrowserHeaders(url);
    const { data }: { data: string } = await axios.get(url, { headers, timeout: 15000 });
    ...
    const base64Array: string[] = arrayMatch[1].split(',').map((s: string) => s.trim().replace(/^"|"$/g, ''));
    ...
    let baseUrl = '';
    base64Array.forEach((val: string) => {
    ...
    throw new Error(`Nupload Extract Error: ${(error as Error).message}`);
  }
}
